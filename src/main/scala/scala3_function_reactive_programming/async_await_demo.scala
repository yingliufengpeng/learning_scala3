...
object async_await_demo {
  ...
  enum Json:
    ...
    def prettyShow(level: Int = 0, isInList :Boolean = false): String =
      this match
        case JObj(bindings) =>
          val firstPrefix = if isInList then   "  " * level + "{\n" else "{\n"
          val middile = bindings.map((k, v) => "  " * (level + 1) + "\"" + k + "\":" + v.prettyShow(level + 1) + ",").mkString("\n")
          val lastPrefix = "\n" + "  " * (level) + "}"
          firstPrefix + middile + lastPrefix
        case JSeq(elems) =>
          val firstPrefix = "[\n"
          val middile = elems.map(e => e.prettyShow(level + 1, true) + ",").mkString("\n")
          val lastPrefix = "\n" + "  " * level + "]"
          firstPrefix + middile + lastPrefix
        case JNum(num) => s"$num"
        case JStr(str) => "\"" + str + "\""
